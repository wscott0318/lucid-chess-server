$( document ).ready(function() {
    ...
    var socket = io('http://localhost:8050', { query: 'user=' + username });
    var tvSocket = io('http://localhost:8050/tv');
    var monitorSocket = io('http://localhost:8050/monitor');
    ...
    if ($("#board").length) {
        ...
        socket.on('wait', function (data) {
            var url = "http:/localhost:8050/game/" + token + "/" + opponentSide;
            $('#gameUrl').html(url);
            $('#gameUrlPopup').modal({
                keyboard: false,
                backdrop: 'static'
